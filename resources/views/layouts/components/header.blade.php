                ...
                <div class="absolute inset-y-0 left-0 flex items-center sm:hidden">
                    ...
                    <button type="button"
                        ...
                        <svg x-description="Icon when menu is closed." x-state:on="Menu open" x-state:off="Menu closed"
                            class="block size-6" :class="{ 'hidden': open, 'block': !(open) }" fill="none"
                        ...
                        <svg x-description="Icon when menu is open." x-state:on="Menu open" x-state:off="Menu closed"
                            class="hidden size-6" :class="{ 'block': open, 'hidden': !(open) }" fill="none"
                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true"
                            data-slot="icon">
                    ...
                    <div x-data="Components.menu({ open: false })" x-init="init()"
                        ...
                        <div x-show="open" x-transition:enter="transition ease-out duration-100"
                            ...
                            <a href="#" class="block px-4 py-2 text-sm text-gray-700" x-state:on="Active"
                                x-state:off="Not Active" :class="{ 'bg-gray-100 outline-hidden': activeIndex === 0 }"
                            ...
                            <a href="#" class="block px-4 py-2 text-sm text-gray-700"
                                :class="{ 'bg-gray-100 outline-hidden': activeIndex === 1 }" role="menuitem"
                                tabindex="-1" id="user-menu-item-1" @mouseenter="onMouseEnter($event)"
                                @mousemove="onMouseMove($event, 1)" @mouseleave="onMouseLeave($event)"
                                @click="open = false; focusButton()">Settings</a>
                            <a href="#" class="block px-4 py-2 text-sm text-gray-700"
                                :class="{ 'bg-gray-100 outline-hidden': activeIndex === 2 }" role="menuitem"
                                tabindex="-1" id="user-menu-item-2" @mouseenter="onMouseEnter($event)"
